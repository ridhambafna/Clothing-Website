...
export function normalizeDbProduct(d: any): Product {
  return {
    ...
    image: (d.image && typeof d.image === "string" && d.image.trim()) ? d.image.trim() : "",
    gallery: (Array.isArray(d.gallery) ? d.gallery : (typeof d.gallery === "string" && d.gallery ? d.gallery.split(",").map((s: string) => s.trim()) : [])).filter((s: string) => typeof s === "string" && s.trim() !== ""),
